  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const found = data.find(
        u => u.umbrella_code === code.toUpperCase()
      )
      ...
      try {
        const statsRes = await fetch(
          `http://127.0.0.1:5000/umbrella-stats?umbrella_id=${found.umbrella_id}`,
          {
            headers: {
        ...
        if (statsRes.ok) {
          stats = {
            studentsHelped: statsData.stats?.students_helped || 0,
            locationsVisited: statsData.stats?.locations_visited || 0,
            daysActive: statsData.stats?.days_active || 0
          }
        }
      } catch {}
  ...
  return (
    <div style={{ fontFamily:'sans-serif', minHeight:'100vh', backgroundColor:'#f0f0f0', paddingBottom:'80px' }}>
      <div style={{ backgroundColor:'#1a3a33', padding:'48px 24px 24px', color:'white' }}>
        <h1 style={{ margin:'0 0 4px', fontSize:'24px', fontWeight:'700', color:'white' }}>
          Borrow an Umbrella
        </h1>
        <p style={{ margin:0, opacity:0.7, fontSize:'14px' }}>
          Enter the code printed on the umbrella handle.
        </p>
      </div>

      <div style={{ padding:'24px', maxWidth:'440px', margin:'0 auto' }}>
        <label style={{ fontSize:'11px', fontWeight:'700', letterSpacing:'1px', color:'#555', display:'block', marginBottom:'8px' }}>
          UMBRELLA CODE
        </label>
        ...
        <div style={{ display:'flex', gap:'8px', marginBottom:'8px' }}>
          ...
          <button
            ...
            style={{ padding:'14px 18px', backgroundColor:'#1a3a33', color:'white', border:'none', borderRadius:'8px', fontSize:'18px', cursor:'pointer' }}
          >
            {loading ? '...' : '→'}
          </button>
        </div>
        ...
        {!umbrella && !error && (
          <div style={{ backgroundColor:'white', borderRadius:'12px', padding:'40px 20px', textAlign:'center' }}>
            ...
            <p style={{ color:'#aaa', fontSize:'14px', margin:0 }}>
              Enter a valid code to see umbrella details
            </p>
          </div>
        )}
        ...
        {umbrella && (
          <div style={{ backgroundColor:'white', borderRadius:'12px', padding:'20px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px' }}>
              <div style={{ width:'44px', height:'44px', backgroundColor:umbrella.colour || '#1a3a33', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'22px' }}>
                ☂️
              </div>
              ...
              <div>
                ...
                <p style={{ margin:0, fontSize:'13px', color:'#888' }}>
                  {umbrella.name} · <span style={{ color:'green', fontWeight:'600' }}>Available</span>
                </p>
              </div>
            </div>
            ...
            <div style={{ display:'flex', gap:'12px', marginBottom:'16px' }}>
              {[
                ...
                <div key={stat.label} style={{ flex:1, backgroundColor:'#f8f8f8', borderRadius:'8px', padding:'12px', textAlign:'center' }}>
                  <p style={{ margin:0, fontWeight:'700', fontSize:'18px', color:'#1a3a33' }}>
                    {stat.value}
                  </p>
